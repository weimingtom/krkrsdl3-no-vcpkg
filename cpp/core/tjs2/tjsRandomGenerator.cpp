    ...
    else if (numparams >= 1)
    {
        if (param[0]->Type() == tvtObject)
        {
            ...
            try
            {
                ...
                // get state array
                TJS_THROW_IF_ERROR(
                    clo.PropGet(TJS_MEMBERMUSTEXIST, TJS_N("state"), NULL, &val, NULL));
                ...
                // get other members
                TJS_THROW_IF_ERROR(
                    clo.PropGet(TJS_MEMBERMUSTEXIST, TJS_N("left"), NULL, &val, NULL));
                data->left = (tjs_int)val;

                TJS_THROW_IF_ERROR(
                    clo.PropGet(TJS_MEMBERMUSTEXIST, TJS_N("next"), NULL, &val, NULL));
...
tTJSNC_RandomGenerator::tTJSNC_RandomGenerator()
  : tTJSNativeClass(TJS_N("RandomGenerator")){
        ...
        TJS_BEGIN_NATIVE_MEMBERS(/*TJS class name*/ RandomGenerator) TJS_DECL_EMPTY_FINALIZE_METHOD
            //----------------------------------------------------------------------
            TJS_BEGIN_NATIVE_CONSTRUCTOR_DECL(/*var. name*/ _this,
                                              /*var. type*/ tTJSNI_RandomGenerator,
