...
static const tjs_char OctPackChar[OctPack_EOT] = {
    'a', 'A', 'b', 'B', 'c', 'C', 'd', 'f', 'h', 'H', 'i', 'I', 'l', 'L',
    'n', 'N', 'p', 'P', 's', 'S', 'v', 'V', 'u', 'w', 'x', 'X', '@', 'm',
};
...
static void AsciiToBin(std::vector<tjs_uint8>& bin,
                       const ttstr& arg,
                       tjs_char fillchar,
                       tjs_int len)
{
...
static void decodeBase64(const ttstr& inbuf, std::vector<tjs_uint8>& outbuf)
{
    ...
    while (dptr < len_4)
    {
        outbuf.push_back(static_cast<tjs_uint8>((base64tonum[data[dptr]] << 2) |
                                                (base64tonum[data[dptr + 1]] >> 4)));
        dptr++;
        outbuf.push_back(static_cast<tjs_uint8>((base64tonum[data[dptr]] << 4) |
                                                (base64tonum[data[dptr + 1]] >> 2)));
        dptr++;
        outbuf.push_back(
            static_cast<tjs_uint8>((base64tonum[data[dptr]] << 6) | (base64tonum[data[dptr + 1]])));
        dptr += 2;
    }
    outbuf.push_back(static_cast<tjs_uint8>((base64tonum[data[dptr]] << 2) |
                                            (base64tonum[data[dptr + 1]] >> 4)));
    ...
    for (tjs_size i = 0; i < count && argindex < numargs; argindex++)
    {
        ...
        switch (t)
        {
            ...
            case OctPack_LONG: // L : 符号無しlong数値(通常4バイト)
                ReadNumber<tjs_uint, tjs_int64, 4, tjs_uint32>(result, args, numargs, argindex,
                                                               len);
                break;
            case OctPack_noshort: // n : unsigned short数値(ネットワークバイトオーダ) network byte
                                  // order short
                ReadNumberBE<tjs_uint16, tjs_int, 2, tjs_uint16>(result, args, numargs, argindex,
                                                                 len);
                break;
            case OctPack_NOLONG: // N : unsigned long数値(ネットワークバイトオーダ) network byte
                                 // order long
                ReadNumberBE<tjs_uint, tjs_int64, 4, tjs_uint32>(result, args, numargs, argindex,
                                                                 len);
                break;
            ...
            case OctPack_SHORT: // S : 符号無しshort数値(通常2バイト) unsign
                ReadNumber<tjs_uint16, tjs_int, 2, tjs_uint16>(result, args, numargs, argindex,
                                                               len);
                break;
            case OctPack_leshort: // v : リトルエンディアンによるunsigned short値 little endian
                                  // short
                ReadNumberLE<tjs_uint16, tjs_int, 2, tjs_uint16>(result, args, numargs, argindex,
                                                                 len);
                break;
            case OctPack_LELONG: // V : リトルエンディアンによるunsigned long値 little endian long
                ReadNumberLE<tjs_uint, tjs_int64, 4, tjs_uint32>(result, args, numargs, argindex,
                                                                 len);
                break;
...
// mtol : true : 上位ビットから下位ビット, false : 下位ビットから上位ビット
// 指定した数値の方が大きくても、その分は無視
static void BinToBitString(
    const tjs_uint8* data, const tjs_uint8* tail, tjs_uint len, ttstr& result, bool mtol)
{
...
static void BinToNumberLE(std::vector<TRet>& result,
                          const tjs_uint8* data,
                          const tjs_uint8* tail,
                          tjs_int len)
{
    if (len < 0)
...
template<typename TRet, int NBYTE>
static void BinToNumberBE(std::vector<TRet>& result,
                          const tjs_uint8* data,
                          const tjs_uint8* tail,
                          tjs_int len)
{
                             ...
                             tjs_uint length)
{
    ...
    tTJSArrayNI* ni;
    if (TJS_FAILED(result->NativeInstanceSupport(TJS_NIS_GETINSTANCE, TJSGetArrayClassID(),
    ...
    for (tjs_int i = 0; i < (tjs_int)count && current < tail; argindex++)
    {
        ...
        switch (t)
        {
            ...
            { // c : 符号付き1バイト数値(-128 ～ 127)
                ...
                for (std::vector<tjs_int8>::const_iterator iter = ret.begin(); iter != ret.end();
                     iter++)
                {
            ...
            { // C : 符号無し1バイト数値(0～255)
                ...
                for (std::vector<tjs_uint8>::const_iterator iter = ret.begin(); iter != ret.end();
                     iter++)
                {
                    result->Add(ni, tTJSVariant((tjs_int)*iter));
                }
            ...
            { // d : 倍精度浮動小数点
                ...
                for (std::vector<tjs_real>::const_iterator iter = ret.begin(); iter != ret.end();
                     iter++)
                {
            ...
            { // f : 単精度浮動小数点
                ...
                for (std::vector<float>::const_iterator iter = ret.begin(); iter != ret.end();
                     iter++)
                {
                    result->Add(ni, tTJSVariant((tjs_real)*iter));
                }
            ...
            { // l : 符号付きlong数値(通常4バイト)
                ...
                for (std::vector<tjs_int>::const_iterator iter = ret.begin(); iter != ret.end();
                     iter++)
                {
                    result->Add(ni, tTJSVariant((tjs_int)*iter));
                }
            ...
            { // L : 符号無しlong数値(通常4バイト)
                ...
                for (std::vector<tjs_uint>::const_iterator iter = ret.begin(); iter != ret.end();
                     iter++)
                {
            ...
            { // n : unsigned short数値(ネットワークバイトオーダ) network byte order short
                if (len < 0)
                    len = (tail - current) / 2;
                std::vector<tjs_uint16> ret;
                BinToNumberBE<tjs_uint16, 2>(ret, current, tail, (tjs_uint)len);
                for (std::vector<tjs_uint16>::const_iterator iter = ret.begin(); iter != ret.end();
                     iter++)
                {
                    result->Add(ni, tTJSVariant((tjs_int)*iter));
                }
            ...
            case OctPack_NOLONG:
            { // N : unsigned long数値(ネットワークバイトオーダ) network byte order long
                if (len < 0)
                    len = ((tail - current) / 4);
                std::vector<tjs_uint> ret;
            ...
            case OctPack_POINTER: // P : 構造体(固定長文字列)へのポインタ fix length char
                ...
                break;
            case OctPack_short:
            { // s : 符号付きshort数値(通常2バイト) sign
                if (len < 0)
                    len = ((tail - current) / 2);
                std::vector<tjs_int16> ret;
                BinToNumber<tjs_int16, 2>(ret, current, tail, (tjs_uint)len);
                for (std::vector<tjs_int16>::const_iterator iter = ret.begin(); iter != ret.end();
                     iter++)
                {
                    result->Add(ni, tTJSVariant((tjs_int)*iter));
                }
            ...
            { // v : リトルエンディアンによるunsigned short値 little endian short
                if (len < 0)
                    len = ((tail - current) / 2);
                std::vector<tjs_uint16> ret;
            ...
            case OctPack_LELONG:
            { // V : リトルエンディアンによるunsigned long値 little endian long
                if (len < 0)
                    len = ((tail - current) / 4);
                std::vector<tjs_uint> ret;
...
tjs_error TJSOctetPack(tTJSVariant** args,
                       tjs_int numargs,
    ...
    if (result)
    {
        ...
        *result = oct;
        if (oct)
            ...
            *result = tTJSVariant((iTJSDispatch2*)NULL, (iTJSDispatch2*)NULL);
    }
    return TJS_S_OK;
}
tjs_error TJSOctetUnpack(const tTJSVariantOctet* target,
                         tTJSVariant** args,
                         tjs_int numargs,
                         tTJSVariant* result)
{
