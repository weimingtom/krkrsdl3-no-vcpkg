...
void tTJSInterCodeContext::Disassemble(
    void (*output_func)(const tjs_char* msg,
    ...
    tjs_int end)
{
    ...
    for (tjs_int i = start; i < end;)
    {
        ...
        switch (CodeArea[i])
        {
            ...
            case VM_CONST:
                msg.printf(TJS_N("const %%%d, *%d"), TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]),
...
#define OP2_DISASM(c, x) \
    ...
    case c + 1: \
        msg.printf(TJS_N(x) TJS_N("pd") TJS_N(" %%%d, %%%d.*%d, %%%d"), \
                   ...
                   TJS_FROM_VM_REG_ADDR(CodeArea[i + 3]), TJS_FROM_VM_REG_ADDR(CodeArea[i + 4])); \
        if (DataArea) \
        { \
...
#undef OP1A_DISASM
            ...
            case VM_NEW:
            {
                ...
                msg.printf(CodeArea[i] == VM_CALL    ? TJS_N("call %%%d, %%%d(")
            ...
            case VM_GPDS:
                // property get direct
                msg.printf(CodeArea[i] == VM_GPD ? TJS_N("gpd %%%d, %%%d.*%d")
                                                 : TJS_N("gpds %%%d, %%%d.*%d"),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 2]),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 3]));
            ...
            case VM_SPDS:
                // property set direct
                msg.printf(CodeArea[i] == VM_SPD     ? TJS_N("spd %%%d.*%d, %%%d")
            ...
            case VM_GPIS:
                ...
                msg.printf(CodeArea[i] == VM_GPI ? TJS_N("gpi %%%d, %%%d.%%%d")
                                                 : TJS_N("gpis %%%d, %%%d.%%%d"),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 2]),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 3]));
            ...
            case VM_SPIS:
                ...
                msg.printf(CodeArea[i] == VM_SPI    ? TJS_N("spi %%%d.%%%d, %%%d")
            ...
            case VM_SETP:
                // property set
                msg.printf(TJS_N("setp %%%d, %%%d"), TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 2]));
                size = 3;
                break;
            ...
            case VM_GETP:
                // property get
                msg.printf(TJS_N("getp %%%d, %%%d"), TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 2]));
                size = 3;
                break;
            ...
            case VM_TYPEOFD:
                ...
                msg.printf(CodeArea[i] == VM_DELD ? TJS_N("deld %%%d, %%%d.*%d")
                                                  : TJS_N("typeofd %%%d, %%%d.*%d"),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 2]),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 3]));
            ...
            case VM_TYPEOFI:
                ...
                msg.printf(CodeArea[i] == VM_DELI ? TJS_N("deli %%%d, %%%d.%%%d")
                                                  : TJS_N("typeofi %%%d, %%%d.%%%d"),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 2]),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 3]));
            ...
            case VM_ENTRY:
                ...
                msg.printf(TJS_N("entry %09d, %%%d"), TJS_FROM_VM_CODE_ADDR(CodeArea[i + 1]) + i,
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 2]));
                size = 3;
                break;
            ...
            case VM_CHGTHIS:
                msg.printf(TJS_N("chgthis %%%d, %%%d"), TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 2]));
                size = 3;
                break;

            case VM_GLOBAL:
                msg.printf(TJS_N("global %%%d"), TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]));
                size = 2;
                break;

            case VM_ADDCI:
                msg.printf(TJS_N("addci %%%d, %%%d"), TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]),
                           TJS_FROM_VM_REG_ADDR(CodeArea[i + 2]));
                size = 3;
                break;
        ...
        output_func(msg.c_str(), com.c_str(), i, CodeArea + i, size, data); // call the callback
...
//---------------------------------------------------------------------------
void tTJSInterCodeContext::Disassemble(void (*output_func)(const tjs_char* msg, void* data),
                                       void* data,
                                       tjs_int start,
                                       tjs_int end)
