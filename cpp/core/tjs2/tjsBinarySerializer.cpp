...
void tTJSBinarySerializer::InsertArray(tTJSArrayObject* array, tjs_uint index, tTJSVariant* value)
{
    if (value == NULL)
        TJS_eTJSError(TJSReadError);
    tTJSArrayNI* ni = NULL;
    tjs_error hr = array->NativeInstanceSupport(TJS_NIS_GETINSTANCE, TJSGetArrayClassID(),
                                                 ...
                                                 tjs_uint& index)
{
    ...
    switch (type)
    {
        ...
        case TYPE_STRING8:
        {
            ...
            index++;
            if ((index + (len * sizeof(tjs_wchar))) > size)
                TJS_eTJSError(TJSReadError);
        ...
        default:
        {
            ...
            else if (type >= TYPE_FIX_RAW_MIN && type <= TYPE_FIX_RAW_MAX)
            { // octet
            ...
            else if (type >= TYPE_FIX_STRING_MIN && type <= TYPE_FIX_STRING_MAX)
            {
                ...
                if ((len * sizeof(tjs_wchar) + index) > size)
                    TJS_eTJSError(TJSReadError);
                return ReadStringVarint(buff, len, index);
            }
    ...
    for (tjs_uint i = 0; i < count; i++)
    {
        ...
        switch (type)
        {
            ...
            default:
                ...
                { // Dictionary形式の場合、最初に文字列がこないといけない
                    TJS_eTJSError(TJSReadError);
                }
                break;
        }
