...
private:
    void Clear();
    static unsigned long IoFunc(FT_Stream stream,
    ...
    try
    {
        if (ft_bmp->rows && ft_bmp->width)
        {
            ...
            if (ft_bmp->pixel_mode != ft_pixel_mode_grays)
            {
                ...
                release_ft_bmp = true;
                ft_bmp = &new_bmp;
                err = FT_Bitmap_Convert(FTFace->glyph->library, &(FTFace->glyph->bitmap), &new_bmp,
                                        1);
                // 結局 tGlyphBitmap
            ...
            if (ft_bmp->num_grays != 256)
            {
                // gray レベルが 256 ではない
                // 256 になるように乗算を行う
                tjs_int32 multiply = static_cast<tjs_int32>((static_cast<tjs_int32>(1) << 30) - 1) /
                                     (ft_bmp->num_grays - 1);
        ...
        int baseline =
            (int)(FTFace->ascender) * FTFace->size->metrics.y_ppem / FTFace->units_per_EM;

        glyph_bmp = new tTVPCharacterData(ft_bmp->buffer, ft_bmp->pitch, FTFace->glyph->bitmap_left,
                                          baseline - FTFace->glyph->bitmap_top, ft_bmp->width,
                                          ft_bmp->rows, metrics);
        glyph_bmp->Gray = 256;
 ...
 */
bool tFreeTypeFace::GetGlyphRectFromCharcode(tTVPRect& rt,
                                             tjs_wchar code,
                                             tjs_int& advancex,
                                             tjs_int& advancey)
{
 ...
 */
bool tFreeTypeFace::GetGlyphMetricsFromCharcode(tjs_wchar code, tGlyphMetrics& metrics)
{
    if (!LoadGlyphSlotFromCharcode(code))
        return false;
