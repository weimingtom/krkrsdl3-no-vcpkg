...
static inline void copy_func_c(tjs_uint32* __restrict dest,
                               const tjs_uint32* __restrict src,
                               tjs_int len)
{
    functor func;
    for (int i = 0; i < len; i++)
...
template<typename functor>
static inline void overlap_blend_func_c(tjs_uint32* dest,
...
#define DEFINE_BLEND_FUNCTION_VARIATION(FUNC) \
    ...
    static void TVP_##FUNC##_HDA_o(tjs_uint32* dest, const tjs_uint32* src, tjs_int len, \
                                   tjs_int opa) \
    { \
...
#define DEFINE_SD_BLEND_FUNCTION(FUNC) \
    static void TVP_##FUNC##_SD(tjs_uint32* dest, const tjs_uint32* src1, const tjs_uint32* src2, \
...
#define DEFINE_APPLY_FUNCTION_VARIATION(FUNC) \
    ...
    static void TVP_##FUNC##_HDA(tjs_uint32* dest, const tjs_uint8* src, tjs_int len, \
                                 tjs_uint32 color) \
    { \
        apply_color_map_func_c<FUNC##_HDA_functor>(dest, src, len, color); \
    } \
    static void TVP_##FUNC##_o(tjs_uint32* dest, const tjs_uint8* src, tjs_int len, \
                               tjs_uint32 color, tjs_int opa) \
    { \
        apply_color_map_o_func_c<FUNC##_o_functor>(dest, src, len, color, opa); \
    } \
    static void TVP_##FUNC##_HDA_o(tjs_uint32* dest, const tjs_uint8* src, tjs_int len, \
                                   tjs_uint32 color, tjs_int opa) \
    { \
        apply_color_map_o_func_c<FUNC##_HDA_o_functor>(dest, src, len, color, opa); \
    } \
    static void TVP_##FUNC##_d(tjs_uint32* dest, const tjs_uint8* src, tjs_int len, \
                               tjs_uint32 color) \
    { \
        apply_color_map_func_c<FUNC##_d_functor>(dest, src, len, color); \
    } \
    static void TVP_##FUNC##_a(tjs_uint32* dest, const tjs_uint8* src, tjs_int len, \
                               tjs_uint32 color) \
    { \
        apply_color_map_func_c<FUNC##_a_functor>(dest, src, len, color); \
    } \
    static void TVP_##FUNC##_do(tjs_uint32* dest, const tjs_uint8* src, tjs_int len, \
                                tjs_uint32 color, tjs_int opa) \
    { \
        apply_color_map_o_func_c<FUNC##_do_functor>(dest, src, len, color, opa); \
    } \
    static void TVP_##FUNC##_ao(tjs_uint32* dest, const tjs_uint8* src, tjs_int len, \
                                tjs_uint32 color, tjs_int opa) \
    { \
...
template<typename functor>
static inline void ch_blur_copy_func(tjs_uint8* dest,
                                     tjs_int destpitch,
                                     tjs_int destwidth,
                                     tjs_int destheight,
                                     const tjs_uint8* src,
    ...
    for (tjs_int y = -blurwidth; y <= blurwidth; y++)
    {
        for (tjs_int x = -blurwidth; x <= blurwidth; x++)
        {
            ...
            if (len <= blurwidth)
            {
                ...
                {
                    blend_func_c(dest + (y + sy + blurwidth) * destpitch + x + blurwidth,
...
void TVP_ch_blur_copy65(tjs_uint8* dest,
                        tjs_int destpitch,
                        tjs_int destwidth,
                        tjs_int destheight,
                        const tjs_uint8* src,
                        ...
                        tjs_int blurlevel)
{
    ch_blur_copy_func<ch_blur_add_mul_copy65_functor>(dest, destpitch, destwidth, destheight, src,
                                                      srcpitch, srcwidth, srcheight, blurwidth,
                                                      blurlevel);
}
void TVP_ch_blur_copy(tjs_uint8* dest,
                      tjs_int destpitch,
                      tjs_int destwidth,
                      tjs_int destheight,
                      const tjs_uint8* src,
                      ...
                      tjs_int blurlevel)
{
    ch_blur_copy_func<ch_blur_add_mul_copy_functor>(dest, destpitch, destwidth, destheight, src,
                                                    srcpitch, srcwidth, srcheight, blurwidth,
                                                    blurlevel);
}
    ...
    for (s = 0; s < 256; s++)
    {
        for (d = 0; d < 256; d++)
        {
            ps_soft_light_table::TABLE[s][d] =
                (s >= 128) ? (((unsigned char)(pow(d / 255.0, 128.0 / s) * 255.0)))
...
extern void TVP_ch_blur_add_mul_copy_sse2_c(tjs_uint8* dest,
                                            ...
                                            tjs_int len,
                                            tjs_int opa);
extern void TVP_ch_blur_mul_copy65_sse2_c(tjs_uint8* dest,
                                          const tjs_uint8* src,
                                          tjs_int len,
                                          tjs_int opa);
extern void TVP_ch_blur_mul_copy_sse2_c(tjs_uint8* dest,
                                        const tjs_uint8* src,
                                        tjs_int len,
                                        tjs_int opa);
/**
