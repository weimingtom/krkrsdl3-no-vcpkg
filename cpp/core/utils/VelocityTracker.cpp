...
LeastSquaresVelocityTrackerStrategy::LeastSquaresVelocityTrackerStrategy(tjs_uint32 degree,
                                                                         Weighting weighting)
  : mDegree(degree),
...
float LeastSquaresVelocityTrackerStrategy::chooseWeight(tjs_uint32 index) const
{
    switch (mWeighting)
    {
        case WEIGHTING_DELTA:
        {
            ...
            // 0.000001f;
            float deltaMillis =
                (float)(mMovements[nextIndex].eventTime - mMovements[index].eventTime);
            if (deltaMillis < 0)
            {
                return 0.5f;
            }
