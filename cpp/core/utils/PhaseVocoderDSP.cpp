...
static inline void VFast_sincos(float v, float& sin, float& cos)
{
    ...
    s1 = VFast_atan2_madd(
        ...
        VFast_atan2_madd(
            q2,
            VFast_atan2_madd(q2, VFast_atan2_madd(q2, (float)(ss4), (float)(ss3)), (float)(ss2)),
            (float)(ss1)),
        (float)(0.0));
    c1 = VFast_atan2_madd(
        q2, VFast_atan2_madd(q2, VFast_atan2_madd(q2, (float)(cc3), (float)(cc2)), (float)(cc1)),
        (float)(1.0));
...
// channels * (framesize + (framesize/oversamp)*MAX_TIME_SCALE) 必要
{
    ...
    try
    {
        ...
        FFTWorkIp = (int*)TJSAlignedAlloc(
            sizeof(int) * (static_cast<int>(2 + sqrt((double)FrameSize / 4))), 4);
...
bool tRisaPhaseVocoderDSP::GetInputBuffer(
    size_t numsamplegranules, float*& p1, size_t& p1size, float*& p2, size_t& p2size)
{
    size_t numsamples = numsamplegranules * Channels;

    if (InputBuffer.GetFreeSize() < numsamples)
...
bool tRisaPhaseVocoderDSP::GetOutputBuffer(
    size_t numsamplegranules, const float*& p1, size_t& p1size, const float*& p2, size_t& p2size)
{
    size_t numsamples = numsamplegranules * Channels;

    if (OutputBuffer.GetDataSize() < numsamples)
...
tRisaPhaseVocoderDSP::tStatus tRisaPhaseVocoderDSP::Process()
{
    ...
    if (RebuildParams)
    {
        ...
        float output_volume = TimeScale / FrameSize / sqrt(FrequencyScale) / OverSampling * 2 *
    ...
    // 窓関数を適用しつつ、SynthWork から出力バッファに書き込む
    {
        ...
        if (p2)
            InterleaveOverlappingWindow(p2, SynthWork, OutputWindow + p1len, Channels, p1len,
                                        p2len);
    }
...
void tRisaPhaseVocoderDSP::ProcessCore(int ch)
{
    ...
    if (FrequencyScale != 1.0)
    {
        ...
        for (unsigned int i = 0; i < framesize_d2; i++)
        {
            ...
            {
                synthwork[i * 2] =
                    analwork[index * 2] + frac * (analwork[index * 2 + 2] - analwork[index * 2]);
                synthwork[i * 2 + 1] =
