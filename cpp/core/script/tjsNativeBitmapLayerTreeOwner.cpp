...
void tTJSNI_BitmapLayerTreeOwner::NotifyBitmapCompleted(class iTVPLayerManager* manager,
                                                        tjs_int x,
                                                        tjs_int y,
                                                        tTVPBaseTexture* bitmapinfo,
...
void tTJSNI_BitmapLayerTreeOwner::OnResizeLayer(tjs_int w, tjs_int h)
{
    if (BitmapNI)
    {
        BitmapNI->SetSize(w, h); // 僒僀僘曄峏偵墳偠偰丄撪晹偺Bitmap傕僒僀僘曄峏偡傞
    }
    if (Owner)
    {
...
tTJSNC_BitmapLayerTreeOwner::tTJSNC_BitmapLayerTreeOwner()
  : inherited(TJS_N("BitmapLayerTreeOwner")){
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ fireMouseDown)
{
    ...
    if (numparams < 4)
        return TJS_E_BADPARAMCOUNT;
    _this->FireMouseDown(*param[0], *param[1], (tTVPMouseButton)(tjs_int)*param[2],
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ fireMouseUp)
{
    ...
    if (numparams < 4)
        return TJS_E_BADPARAMCOUNT;
    _this->FireMouseUp(*param[0], *param[1], (tTVPMouseButton)(tjs_int)*param[2],
                       (tjs_uint32)(tjs_int64)*param[3]);
    return TJS_S_OK;
}
...
if (!pri)
    TVPThrowExceptionMessage(TJS_N("Not have primary layer"));
