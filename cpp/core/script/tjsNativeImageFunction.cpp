...
tTVPBBBltMethod tTJSNC_ImageFunction::GetBltMethodFromOperationMode(tTVPBlendOperationMode mode,
                                                                    tTVPDrawFace face)
{
...
bool tTJSNC_ImageFunction::ClipDestPointAndSrcRect(tjs_int& dx,
                                                   tjs_int& dy,
                                                   tTVPRect& srcrectout,
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ operateAffine)
{
    ...
    if (clo.Object)
    {
        if (TJS_FAILED(clo.Object->NativeInstanceSupport(
                TJS_NIS_GETINSTANCE, tTJSNC_Bitmap::ClassID, (iTJSNativeInstance**)&dst)))
    ...
    {
        if (TJS_FAILED(clo.Object->NativeInstanceSupport(
                TJS_NIS_GETINSTANCE, tTJSNC_Bitmap::ClassID, (iTJSNativeInstance**)&src)))
            return TJS_E_INVALIDPARAM;
    }
    if ((!dst) || (!src))
        return TJS_E_INVALIDPARAM;
    ...
    if (numparams >= 9 && param[8]->Type() == tvtObject)
    {
        ...
        {
            if (TJS_FAILED(clo.Object->NativeInstanceSupport(
                    TJS_NIS_GETINSTANCE, tTJSNC_Rect::ClassID, (iTJSNativeInstance**)&rect)))
                return TJS_E_INVALIDPARAM;
            srcRect = rect->Get();
        }
    }
    ...
    if (affine)
    {
        ...
        mat.tx = *param[6];
        mat.ty = *param[7];
        updated = dst->GetBitmap()->AffineBlt(clipRect, src->GetBitmap(), srcRect, mat, met, opa,
                                              &updaterect, hda, type);
    }
    else
    {
        ...
        points[2].x = *param[6];
        points[2].y = *param[7];
        updated = dst->GetBitmap()->AffineBlt(clipRect, src->GetBitmap(), srcRect, points, met, opa,
                                              &updaterect, hda, type);
    }
    if (result)
    {
        if (updated)
        {
            iTJSDispatch2* ret = TVPCreateRectObject(updaterect.left, updaterect.top,
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ operateStretch)
{
    ...
    {
        tTVPBBBltMethod met = GetBltMethodFromOperationMode(mode, face);
        updated = dst->GetBitmap()->StretchBlt(clipRect, dstRect, src->GetBitmap(), srcRect, met,
                                               opa, hda, type, typeopt);
    }
    if (result)
    {
        if (updated)
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ flipLR)
{
    ...
    {
        iTJSDispatch2* ret =
            TVPCreateRectObject(dstRect.left, dstRect.top, dstRect.right, dstRect.bottom);
        *result = tTJSVariant(ret, ret);
        ret->Release();
    }
...
{
    // bmp, rgamma=1.0, rfloor=0, rceil=255, ggamma=1.0, gfloor=0, gceil=255, bgamma=1.0, bfloor=0,
    // bceil=255, cliprect=null, isaddalpha=false cliprect=null, isaddalpha=false
    if (numparams < 1)
        return TJS_E_BADPARAMCOUNT;
    if (numparams == 1)
        return TJS_S_OK;
    ...
    {
        iTJSDispatch2* ret =
            TVPCreateRectObject(clipRect.left, clipRect.top, clipRect.right, clipRect.bottom);
        *result = tTJSVariant(ret, ret);
        ret->Release();
    }
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ doBoxBlur)
{
    ...
    if (isalpha == false)
        ...
        updated =
            dst->GetBitmap()->DoBoxBlurForAlpha(clipRect, tTVPRect(-xblur, -yblur, xblur, yblur));
...
TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ fillRect)
{
    ...
    if (numparams >= 3 && param[2]->Type() == tvtObject)
    {
        ...
        {
            if (TJS_FAILED(clo.Object->NativeInstanceSupport(
                    TJS_NIS_GETINSTANCE, tTJSNC_Rect::ClassID, (iTJSNativeInstance**)&rectni)))
                return TJS_E_INVALIDPARAM;
            rect = rectni->Get();
        }
    }
    ...
    if (result)
    {
        ...
        {
            iTJSDispatch2* ret =
                TVPCreateRectObject(destrect.left, destrect.top, destrect.right, destrect.bottom);
            *result = tTJSVariant(ret, ret);
            ret->Release();
        }
        else
...
{
    // bmp, font, x, y, text, color, opa=255, aa=true, face=dfAlpha, shadowlevel=0,
    // shadowcolor=0x000000, shadowwidth=0, shadowofsx=0, shadowofsy=0, hda=false, clipRect=null
    if (numparams < 6)
        return TJS_E_BADPARAMCOUNT;
    ...
    tjs_int opa =
        (numparams >= 7 && param[6]->Type() != tvtVoid) ? (tjs_int)*param[6] : (tjs_int)255;
    ...
    tjs_uint32 shadowcolor = (numparams >= 11 && param[10]->Type() != tvtVoid)
                                 ...
                                 : 0;
    tjs_int shadowwidth =
        (numparams >= 12 && param[11]->Type() != tvtVoid) ? (tjs_int)*param[11] : 0;
    tjs_int shadowofsx =
        (numparams >= 13 && param[12]->Type() != tvtVoid) ? (tjs_int)*param[12] : 0;
    tjs_int shadowofsy =
        (numparams >= 14 && param[13]->Type() != tvtVoid) ? (tjs_int)*param[13] : 0;
    bool hda =
        (numparams >= 15 && param[14]->Type() != tvtVoid) ? param[14]->operator bool() : false;
    ...
    dst->GetBitmap()->DrawText(clipRect, x, y, text, color, met, opa, hda, aa, shadowlevel,
...
{
    // bmp, x, y, glyph, color, opa=255, aa=true, face=dfAlpha, shadowlevel=0, shadowcolor=0x000000,
    // shadowwidth=0, shadowofsx=0, shadowofsy=0, hda=false, clipRect=null
    if (numparams < 5)
        return TJS_E_BADPARAMCOUNT;
    ...
    tjs_int opa =
        (numparams >= 6 && param[5]->Type() != tvtVoid) ? (tjs_int)*param[5] : (tjs_int)255;
    ...
    tjs_int shadowofsy =
        (numparams >= 13 && param[12]->Type() != tvtVoid) ? (tjs_int)*param[12] : 0;
    bool hda =
        (numparams >= 14 && param[13]->Type() != tvtVoid) ? param[13]->operator bool() : false;
    ...
    color = TVPToActualColor(color);
    tTVPComplexRect r;
    dst->GetBitmap()->DrawGlyph(glyph, clipRect, x, y, color, met, opa, hda, aa, shadowlevel,
                                shadowcolor, shadowwidth, shadowofsx, shadowofsy, &r);
