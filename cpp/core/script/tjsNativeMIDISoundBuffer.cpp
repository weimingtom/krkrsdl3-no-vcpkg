...
void tTVPSMFTrack::ProcessEvent()
{
    ...
    static tjs_uint8 sysex_modeset[] = {0xf0, 0x41, 0x10, 0x42,
        ...
        switch (ev >> 4)
        {
            case 0x9:
                ...
                if (*Position < 128)
                {
                    // check "on" notes
                    if (Position[1] != 0)
                        Owner->UsingNote[ev & 0x0f][*Position / 32] |= (tjs_uint32)1
                                                                       << (*Position % 32);
                    else
                ...
                (dlen >= 8 && !memcmp(data, sysex_xgon, 9)))
            {
                ...
                for (i = 0; i < 16; i++)
                {
                    ...
                    TVPMIDIOut((tjs_uint8)(0xb0 + i), 0x07,
                               (tjs_uint8)(Owner->Volumes[i] * Owner->BufferVolume / 127));
                }
            }
...
void tTJSNI_MIDISoundBuffer::Open(const ttstr& name)
{
    ...
    try
    {
        ...
        if (format != 0 && format != 1)
            TVPThrowExceptionMessage(TVPInvalidSMF, TJS_N("Format must be 0 or 1"));
        ...
        for (tjs_int i = 0; i < trackcount; i++)
        {
            ...
            if (memcmp(buf, "MTrk", 4))
                TVPThrowExceptionMessage(TVPInvalidSMF, TJS_N("MTrk not found"));
            pb = buf + 4;
  ...
  : tTJSNativeClass(TJS_N("MIDISoundBuffer")){
        ...
        TJS_DECL_EMPTY_FINALIZE_METHOD
            ...
            TJS_BEGIN_NATIVE_CONSTRUCTOR_DECL(/*var.name*/ _this,
                                              ...
                                              /*TJS class name*/ MIDISoundBuffer){return TJS_S_OK;
}
