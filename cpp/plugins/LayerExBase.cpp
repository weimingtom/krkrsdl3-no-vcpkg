...
void NI_LayerExBase::init(iTJSDispatch2* layerobj)
{
    ...
    if (TJS_FAILED(layerobj->PropGet(TJS_IGNOREPROP, TJS_N("mainImageBufferForWrite"), NULL, &var,
                                     layerobj)))
    {
    ...
    if (TJS_FAILED(
            layerobj->PropGet(TJS_IGNOREPROP, TJS_N("mainImageBufferPitch"), NULL, &var, layerobj)))
    {
...
NI_LayerExBase* NI_LayerExBase::getNative(iTJSDispatch2* objthis, bool create)
{
    ...
    if (TJS_FAILED(objthis->NativeInstanceSupport(TJS_NIS_GETINSTANCE, classId,
                                                  (iTJSNativeInstance**)&_this)) &&
        create)
    {
        _this = new NI_LayerExBase();
        if (TJS_FAILED(objthis->NativeInstanceSupport(TJS_NIS_REGISTER, classId,
                                                      (iTJSNativeInstance**)&_this)))
        {
            delete _this;
            _this = NULL;
